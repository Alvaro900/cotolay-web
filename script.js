...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- 3D Hologram Hover Effect for Especialidades ---
    ...
    especialidadCards.forEach(card => {
        card.addEventListener('mousemove', e => {
            ...
            const img = card.querySelector('.especialidad-img img');
            if(img) {
                const shadowX = (x - centerX) / 8;
                const shadowY = (y - centerY) / 8;
                img.style.filter = `drop-shadow(${-shadowX}px ${-shadowY}px 28px rgba(0,0,0,0.4))`;
            }
        });
        ...
        card.addEventListener('mouseleave', () => {
            ...
            if(img) {
                ...
                img.style.transition = 'filter 0.6s cubic-bezier(0.23, 1, 0.32, 1)';
            }
        });
